...
private:
  std::list< std::function<void()> > _execute_list;
};
...
void calc::impl_( ReqPtr req, Callback cb, F f)
{
  ...
  _execute_list.push_back([preq, cb, f]
  {
    auto res = std::make_unique<Res>();
    res->value = f( (*preq)->first, (*preq)->second);
    cb( std::move(res) );
  });
}
...
public:
  ...
  void unreg_io(wjrpc::io_id_t io_id)
  {
    _jsonrpc->unreg_io(io_id);
  }
